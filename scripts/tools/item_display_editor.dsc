...
item_display_editor_command:
    ...
    - else if <[argument]> == copy:
        - give selitem_display_editor_item
        - narrate "<&[base]>"
    - else:
        - narrate "<&[base]>Syntax: <&[emphasis]>/ide [spawn|gui|item|copy]"
...
############# copy display entities

selitem_display_editor_item:
    type: item
    material: blaze_rod
    debug: false
    display name: <white>Copy Selection Display Editor
    lore:
    - <gold>Swapping items(F)
    - <&[base]>Paste the Selection.
    - <gold>Left click
    - <&[base]>Select first corner.
    - <gold>Right click
    - <&[base]>Select second corner.